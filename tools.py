...
def parse_proxy_request(payload, url, req_type,debug = False):
    if debug:
        ...
        print (url)
    if req_type is "post":
        return requests.post(url, params=payload.args, data=payload.data, headers=payload.headers)
    elif req_type is "delete":
        return requests.delete(url, params=payload.args, data=payload.data, headers=payload.headers)
